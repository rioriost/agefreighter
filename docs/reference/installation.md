...
`vVERSION` includes the leading `v`, for example `v2.3.1`.

The Windows archive is included in v2.3.1, but its executables are intentionally
...
```sh
grep 'agefreighter_v2.3.1_darwin_arm64.tar.gz$' checksums.txt |
  shasum -a 256 -c -
```
...
```sh
gh attestation verify agefreighter_v2.3.1_darwin_arm64.tar.gz \
...
The v2.3.1 Windows executables are unsigned. Verify their ZIP checksum and
...
`agefreighter-tools.exe` in a directory on `PATH`. For v2.3.1,
...
The stable release also contains `agefreighter-2.3.1.vsix`, its SPDX SBOM, and
the VSIX checksum in `checksums.txt`. The same GitHub provenance attestation
covers the exact package. Normally, install the public Marketplace listing. For
offline or prepublication verification:

```sh
grep 'agefreighter-2.3.1.vsix$' checksums.txt | shasum -a 256 -c -
code --install-extension agefreighter-2.3.1.vsix
```

The VSIX does not bundle a CLI. Install the matching platform archive or
Homebrew Formula first. See the [VS Code extension guide](vscode-extension.md).
