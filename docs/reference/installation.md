...
`vVERSION` includes the leading `v`, for example `v2.3.0`.

The Windows archive is included in v2.3.0, but its executables are intentionally
...
```sh
grep 'agefreighter_v2.3.0_darwin_arm64.tar.gz$' checksums.txt |
  shasum -a 256 -c -
```
...
```sh
gh attestation verify agefreighter_v2.3.0_darwin_arm64.tar.gz \
...
The v2.3.0 Windows executables are unsigned. Verify their ZIP checksum and
...
`agefreighter-tools.exe` in a directory on `PATH`. For v2.3.0,
...
The stable release also contains `agefreighter-2.3.0.vsix`, its SPDX SBOM, and
...
```sh
grep 'agefreighter-2.3.0.vsix$' checksums.txt | shasum -a 256 -c -
