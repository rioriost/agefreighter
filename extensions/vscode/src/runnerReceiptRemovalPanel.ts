...
export async function manageReadinessRemoval(control: RunnerControl, store: RunnerStore, azure: AzureSession): Promise<void> {
  ...
  trust();
  const selected = await vscode.window.showQuickPick((await store.list()).filter(r => r.readinessReceipts?.length).map(r => ({ label: r.id, description: r.input.resourceGroup, id: r.id })), { placeHolder: "Select retained runner readiness evidence" });
  if (!selected) return;
  ...
  const choice = await vscode.window.showQuickPick((record.readinessReceipts ?? []).map(receipt => {
    ...
    return { label: receipt.command.id.slice(receipt.command.id.lastIndexOf("/") + 1), description: intent ? `GET-only reconciliation: ${intent.phase}` : "Review one old readiness control (requires deallocated VM)", receipt, intent };
  ...
  if (await vscode.window.showWarningMessage("Archive and permanently remove this one Azure readiness control record?", { modal: true,
    detail: `${plan.commandId}\nReceipt SHA-256: ${plan.receiptSHA256}\nOnly this successful, unreferenced readiness record is removed. VM must already be deallocated. Disks, guest evidence, jobs and source data are preserved. A verified local archive and single-use intent are saved first. Coordinate exclusive control: do not modify this command or VM from another client during this step. Pending or changed evidence blocks removal. Lost replies are reconciled by GET only, never an automatic DELETE retry. This does not start compute or a migration.` }, "Archive and remove this record") !== "Archive and remove this record") return;
