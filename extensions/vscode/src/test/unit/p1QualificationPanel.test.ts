...
function fixture(change:(value:any)=>void=()=>{}, transport="valid") {
  ...
  return {run:()=>output.exports.qualifyP1({},control,store,{reportCapability:async()=>capability()},id),record:()=>record,snapshots,panels,requests,retained};
}
...
const invalidReports:[string,(r:any)=>void][]=[
  ["foreign envelope job",r=>{r.jobId=foreign;}],
  ["foreign target job",r=>{r.actual.jobId=foreign;}],
  ["missing range",r=>{r.actual.leaves.pop();}],
  ["duplicated range",r=>{r.actual.leaves[1]=r.actual.leaves[0];}],
  ["reordered ranges",r=>{r.actual.leaves.reverse();}],
  ["forged matching summaries and leaves",r=>{r.expected.leaves[0].sha256="a".repeat(64);r.actual.leaves[0].sha256="a".repeat(64);}],
  ["wrong fixture root",r=>{r.actual.fixtureRootSha256="a".repeat(64);}],
  ["unknown canonical version",r=>{r.actual.canonicalVersion="future";}],
  ["different record count",r=>{r.actual.recordCount--;}],
  ["comparison failure",r=>{r.comparison.status="fail";}],
  ["non-read-only receipt",r=>{r.readOnly=false;}]
];
for(const [name,change] of invalidReports)test(`P1 controller never persists or displays PASS for ${name}`,async()=>{
...
for(const kind of ["hash","length","truncated","403","throw"])test(`P1 controller refuses ${kind} transfer before retention or success`,async()=>{
  ...
  await assert.rejects(f.run(),/Report download could not be verified/);
