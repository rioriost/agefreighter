...
import { LostResponseSetupRenewal, validateLostResponseRenewal } from "./deploymentLostResponseRenewal";
...
export async function prepareLostResponseCompanion(root:string,extensionRoot:string,scope:LostResponseScope,continuationRoot?:string,renewal?:LostResponseSetupRenewal):Promise<string[]> {
  ...
  let preservedStorageIntent:Buffer|undefined;
  let renewalReceipt:unknown={schemaVersion:1,kind:"none"};
  assert.ok(!renewal||continuationRoot,"Renewal requires the prior private root");
  if(continuationRoot){
    ...
    const previousScopeBytes=await readFile(join(continuationRoot,"scope.json")),previousScope=JSON.parse(previousScopeBytes.toString()) as LostResponseScope;
    if(renewal)validateLostResponseRenewal(renewal,previousScope,scope);
    else assert.equal(sha(previousScopeBytes),sha(Buffer.from(JSON.stringify(scope,null,2)+"\n")));
    const ledger=await readdir(join(continuationRoot,"ledger"));
    assert.ok(ledger.every(n=>/^stage-\d{4}\.json$/.test(n)||(renewal&&n==="storage-put-intent.json")),"No other effect intent may precede continuation");
    const ledgerSHA256:Record<string,string>={};
    if(renewal){
      assert.ok(ledger.includes("storage-put-intent.json"));
      const claimPath=join(continuationRoot,"ledger/storage-put-intent.json"),info=await lstat(claimPath);
      assert.ok(info.isFile()&&!info.isSymbolicLink()&&info.size<16384);assert.equal(info.mode&0o777,0o600);
      preservedStorageIntent=await readFile(claimPath);
    }
    for(const name of ledger){const bytes=await readFile(join(continuationRoot,"ledger",name));ledgerSHA256[name]=sha(bytes);if(name==="storage-put-intent.json")continue;const e=JSON.parse(bytes.toString());assert.ok(["subscriptions","locations","storagePrincipal","LIST","GET","POST-whatIf","whatIf-poll-header"].includes(e.kind));}
    ...
    seed=await readFile(sourceFile);validateLostResponseSeed(JSON.parse(seed.toString()) as RunnerRecord,scope,preservedStorageIntent?JSON.parse(preservedStorageIntent.toString()):undefined);
    continuation={root:continuationRoot,recordSHA256:sha(seed),ledgerFiles:ledger,ledgerSHA256,cloudEffectsObserved:renewal?"one prior storage PUT":false,storageRoleWillBeReviewedAgain:!renewal};
    if(renewal){
      renewalReceipt={schemaVersion:1,kind:"ready-storage-renewal",renewal,previousScope,priorRoot:continuationRoot,priorScopeSHA256:sha(previousScopeBytes),priorLedgerSHA256:ledgerSHA256,initialRecordSHA256:sha(seed),storagePutIntentSHA256:sha(preservedStorageIntent!)};
    }
  }
  ...
  const renewalBytes=Buffer.from(JSON.stringify(renewalReceipt,null,2)+"\n"),renewalSHA256=sha(renewalBytes);
  await write("renewal.json",renewalBytes);
  const storagePutIntentSHA256=preservedStorageIntent?sha(preservedStorageIntent):undefined;
  if(preservedStorageIntent)await write("preserved-storage-put-intent.json",preservedStorageIntent);
  ...
  await retainLostResponseFile(root,"preparation.json",{bundleSHA256,scopeSHA256,initialRecordSHA256,renewalSHA256,storagePutIntentSHA256,continuation,sourceHashes,args,cloudExecuted:false,nativeQualified:false,sourceDispatchPermitted:false,approvalRequiredBeforeLaunch:true});return args;
}
