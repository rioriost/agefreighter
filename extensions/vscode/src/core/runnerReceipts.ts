...
export function readinessReceiptReferenced(record: RunnerRecord, receipt: ReadinessReceipt): boolean {
  ...
  const catalogReady = record.postgresCatalog?.readiness;
  if (catalogReady?.checkedAt === receipt.readiness.checkedAt && catalogReady.bootId === receipt.readiness.bootId) return true;
