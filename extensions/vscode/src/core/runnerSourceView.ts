...
<p>Source data must remain unchanged. Discovery and ordered reads may scan more data than the 10,000-row sample. Cosmos reads consume RU. Verify the runner once and review this form. After credential entry, stale Linux readiness is refreshed automatically without reading source data. An unhealthy or changed boot blocks execution; credentials are not saved.</p><button id="assess" disabled>Approve sampled assessment</button><button id="inventory" disabled>Approve complete source inventory</button></section>
...
<section><h2>Retained operation</h2><pre id="assessment" role="status"></pre><button id="refresh">Refresh assessment status</button><button id="report" disabled>Transfer / open verified report</button><pre id="transferStatus" role="status"></pre><p class="muted">First transfer starts a create-only export; select again to reconcile and import the exact bytes. A finished worker or imported report is not a successful migration. Capacity acceptance, target deployment and migration remain separate gates.</p></section>
...
el('storage').addEventListener('click',()=>send('storage'));el('report').addEventListener('click',()=>send('report'));
