...
export function credentialBinding(r: RunnerRecord, c: SourceConnection): string {
  ...
  const failures = [...r.assessmentHistory ?? [], ...r.migrationHistory?.map(x => x.migration) ?? []]
    .filter(x => ["failed", "interrupted"].includes(x.phase)).map(x => x.operation);
...
export async function savedSourceCredential(vault: SecretVault, r: RunnerRecord, c: SourceConnection, now = Date.now()): Promise<string | undefined> {
  ...
  if ([r.assessment, r.postgresCatalog, r.migration].some(op => op && ["failed", "interrupted"].includes(op.phase))) {
    await vault.delete(credentialKey(r.id)); return undefined;
  }
  try {
