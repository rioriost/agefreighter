...
function update(){el('deploy').disabled=busy||!record||record.phase!=='previewed'||!el('networkApproved').checked||!el('costApproved').checked;el('refreshPlacement').disabled=busy||!el('runnerSubscription').value;el('preview').disabled=busy||!el('runnerGroup').value||!el('region').value;}
function setBusy(value){busy=value;for(const n of document.querySelectorAll('button,input,select'))n.disabled=busy;document.querySelector('section:last-of-type button').disabled=true;el('activity').textContent=busy?'Checking…':'';update();}
...
for(const id of ['restore','csv','refresh'])el(id).addEventListener('click',()=>send(id));
