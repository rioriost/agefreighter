...
<label>Migration resource group (existing)<select id="runnerGroup"><option value="">Select runner subscription first</option></select></label><label>Azure region<select id="region"><option value="">Select runner subscription first</option></select></label><label>Availability zone<select id="zone"><option>1</option><option>2</option><option>3</option></select></label>
...
<p class="muted">Need a new resource group? Create it in Azure, then refresh this list. A resource group is not a network boundary: a VNet may be in another group. Peering is not required merely because groups differ. The VM uses a compute subnet; private-access Flexible Server will require a separate delegated subnet. Target deployment is not yet enabled.</p>
...
function update(){el('deploy').disabled=busy||!record||record.phase!=='previewed'||!el('networkApproved').checked||!el('costApproved').checked;el('refreshPlacement').disabled=busy||!el('runnerSubscription').value;el('preview').disabled=busy||!el('runnerGroup').value||!el('region').value;el('guestReady').disabled=busy||!record||record.phase!=='provisioned'||['submitted','unknown'].includes(record.guestCommand?.phase);el('guestRefresh').disabled=busy||!record?.guestCommand;el('assessmentDisabled').disabled=true;}
function setBusy(value){busy=value;for(const n of document.querySelectorAll('button,input,select'))n.disabled=busy;el('activity').textContent=busy?'Checking…':'';update();}
...
function resetPlacement(){options('runnerGroup',[],'Select runner subscription first');options('region',[],'Select runner subscription first');el('subnet').value='';invalidate();}
function resetSource(){el('sourceId').value='';candidates=[];options('candidate',[],'Discover candidates first');}
...
el('candidate').addEventListener('change',()=>{const c=candidates.find(v=>v.id===el('candidate').value);if(c){el('sourceId').value=c.id;suggestSourceRegion();if(c.zone)el('zone').value=c.zone;}else{el('sourceId').value='';}});
...
window.addEventListener('message',event=>{const m=event.data;if(m.kind==='busy')setBusy(m.value);
...
if(m.kind==='record')el('guestStatus').textContent=m.record.guestCommand?.phase==='failed'?'Guest readiness could not be verified. Retained evidence must be reviewed.':m.record.guestReady?'Pinned Linux guest verified at '+m.record.guestReady.checkedAt+'; source assessment is still required.':m.record.guestCommand?'Guest command '+m.record.guestCommand.phase+' — refresh to reconcile.':'Guest readiness has not been checked.';
