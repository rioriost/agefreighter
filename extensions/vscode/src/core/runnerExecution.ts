...
import {CSVTargetEvidence,csvTargetEvidence,targetBudget} from "./runnerTarget";
...
export interface RunnerMigration {
  ...
  evidence:CSVTargetEvidence; guestConfigurationSHA256?:string; fingerprint?:string;
...
export async function migrationPreflight(control:RunnerControl,r:RunnerRecord,report:string):Promise<CSVTargetEvidence>{
  ...
  if(!r.guestReady?.capabilities?.includes("csv-migration-v1"))throw new Error("Upgrade to a reviewed migration-capable Linux artifact and repeat complete CSV inventory before migration.");
...
export async function applyTargetPreload(control:RunnerControl,r:RunnerRecord,approved=false):Promise<RunnerRecord>{
  ...
  if(response.status!==200 || tags.workflow!==r.id || tags.application!=="agefreighter" || tags.purpose!=="csv-migration-target")throw new Error("Target ownership changed.");
...
export async function startMigration(control:RunnerControl,r:RunnerRecord,report:string,password:string):Promise<RunnerRecord>{
  ...
  return dispatchGuest(control,{...r,migration},{version:1,workflow:r.id,operation,action:"migrate-csv",configuration:r.sourceDraft!.configuration,secrets:{AGEFREIGHTER_TARGET_DSN:targetDSN(r,password)}});
}
export async function refreshMigration(control:RunnerControl,r:RunnerRecord):Promise<RunnerRecord>{
  ...
  if(r.guestCommand!.operation!==m.operation || !["migrate-csv","status"].includes(r.guestCommand!.action))throw new Error("Reconcile the other pending guest command first.");
  const checked=await reconcileGuest(control,r);if(!checked.result)return checked.record;
  const s=object(checked.result);
  if(s.jobId!==m.jobId || s.action!=="migrate-csv" || s.bootId!==m.bootId || typeof s.configSha256!=="string" || !sha.test(s.configSha256) || m.guestConfigurationSHA256 && m.guestConfigurationSHA256!==s.configSha256 || !["accepted","running","finished","failed","interrupted"].includes(String(s.phase)))throw new Error("Retained migration identity changed.");
...
export function verifyMigrationReport(r:RunnerRecord,text:string):RunnerRecord{
  ...
  const verification=assessCountsVerification({jobId:m.jobId,fingerprint:m.fingerprint,cliVersion:m.cliVersion,startedAt:m.startedAt,labels:m.evidence.labels},{exitCode:m.exitCode,reportJSON:text,sha256:m.reportSHA256});
