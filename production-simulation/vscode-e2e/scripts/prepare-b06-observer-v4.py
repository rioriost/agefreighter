...
OBSERVER = HERE / "b06-observe-access-v4.py"
spec = importlib.util.spec_from_file_location("b06_v4_observer", OBSERVER)
...
def body(config, observer_sha):
    ...
    source = "\n".join([
        "set -eu", "exec /usr/bin/python3 - <<'AF_B06_V4'",
        ...
        "    module = types.ModuleType('b06_v4_pinned_observer')",
        "    exec(compile(code, 'b06_v4_pinned_observer', 'exec'), module.__dict__)",
...
def prepare(binding_path, binding_sha, identity_path, identity_sha, observer_sha, output):
    ...
    for config, request, config_raw in prepared:
        ...
        name = f"af-b06-v4-{phase}-{config['attempt']:02d}"
