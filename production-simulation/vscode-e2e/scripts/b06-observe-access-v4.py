...
SHA = r"[0-9a-f]{64}"
QUERY = {"query": "SELECT VALUE 1 FROM c", "parameters": []}
...
def validate(config, clock=now):
    ...
    require(approved <= first <= observed < expiry and expiry - first <= dt.timedelta(hours=4)
            and expiry - approved <= dt.timedelta(hours=96))
...
# Extract a single grammatical denial clause, not unrelated identifiers found
# anywhere in a message. Unknown principal/action strings are never emitted.
DENIAL = re.compile(
    r"\bprincipal\s+\[(?P<principal>" + UUID + r")\]\s+"
    r"(?P<negation>does not|doesn't)\s+have\s+(?P<article>the\s+)?required\s+RBAC\s+permissions\s+"
    r"to\s+perform\s+action\s+\[(?P<action>[A-Za-z][A-Za-z0-9./_-]{0,255})\]",
    re.IGNORECASE)


def service_evidence(value, principal):
    evidence = {"parserVersion": "same-clause-v4", "parseStatus": "supported", "messageFieldPaths": [], "denialClauseCount": 0,
                "samePrincipalClauseFound": False, "otherPrincipalClauseFound": False,
                "unrecognizedActionClauseFound": False, "strictMatch": False,
                "rbacPhraseMentioned": False, "approvedPrincipalMentioned": False,
                "allowlistedReadActionMentioned": False}
    messages = []
    for key in ("message", "Message"):
        if key in value:
            if not isinstance(value[key], str):
                evidence["parseStatus"] = "unsupported-message-shape"
                return evidence
            messages.append((key, value[key]))
    if "Errors" in value:
        errors = value["Errors"]
        if not isinstance(errors, list) or len(errors) > 8 or any(not isinstance(x, str) for x in errors):
            evidence["parseStatus"] = "unsupported-message-shape"
            return evidence
        messages.extend(("Errors[" + str(i) + "]", text) for i, text in enumerate(errors))
    if not messages:
        evidence["parseStatus"] = "no-supported-message-field"
        return evidence
    evidence["messageFieldPaths"] = [path for path, _ in messages]
    proof = None
    for path, message in messages:
        # Diagnostic mentions are independent booleans, never evidence that the
        # identifiers/actions belong to the same denial. Only DENIAL can prove it.
        evidence["rbacPhraseMentioned"] |= bool(re.search(r"\brequired\s+RBAC\s+permissions\b", message, re.IGNORECASE))
        evidence["approvedPrincipalMentioned"] |= bool(re.search(r"(?<![0-9a-f-])" + re.escape(principal) + r"(?![0-9a-f-])", message, re.IGNORECASE))
        evidence["allowlistedReadActionMentioned"] |= any(action.lower() in message.lower() for action in READ_ACTIONS)
        for match in DENIAL.finditer(message):
            evidence["denialClauseCount"] += 1
            if evidence["denialClauseCount"] > 8:
                evidence["parseStatus"] = "too-many-denial-clauses"
                return evidence
            observed_principal = match["principal"].lower()
            same = observed_principal == principal.lower()
            evidence["samePrincipalClauseFound"] |= same
            evidence["otherPrincipalClauseFound"] |= not same
            action = next((x for x in READ_ACTIONS if x.lower() == match["action"].lower()), None)
            evidence["unrecognizedActionClauseFound"] |= action is None
            if same and action and proof is None:
                # These values come from the same matched response clause. The
                # identity is emitted only after equality with the approved ID;
                # action is a canonical fixed allowlist value, never free text.
                proof = {"fieldPath": path, "servicePrincipalId": observed_principal,
                         "readAction": action,
                         "decodedClauseSHA256": hashlib.sha256(match[0].encode()).hexdigest(),
                         "phraseVariant": ("does-not" if match["negation"].lower() == "does not" else "doesnt")
                         + ("-have-the-required" if match["article"] else "-have-required")}
    if proof and not evidence["otherPrincipalClauseFound"] and not evidence["unrecognizedActionClauseFound"]:
        evidence.update(strictMatch=True, proof=proof)
    return evidence
...
def classify(status, headers, body, principal):
    ...
    try:
        ...
        if not isinstance(value, dict):
            if status == 403:
                result["rbacEvidence"] = {"parseStatus": "unsupported-json-shape", "strictMatch": False}
            return result
        if status == 200 and value.get("Documents") == [1] and type(value["Documents"][0]) is int:
            ...
            evidence = service_evidence(value, principal)
            result["rbacEvidence"] = evidence
            if evidence["strictMatch"]:
                result.update(classification="rbac-denied", servicePrincipalMatches=True, deniedReadActionPresent=True)
    except (ValueError, TypeError):
        if status == 403:
            result["rbacEvidence"] = {"parseStatus": "invalid-json", "strictMatch": False}
    return result
