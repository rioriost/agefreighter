...
| B05 | Cosmos explicit and Gremlin documents | Explicit-document base P1 passed; offline P1/source preparation pass; target verifier checks composite IDs/endpoints; corrected Linux runner installed and healthy; actual retry preparation blocked by extension same-boot retention defect; TypeScript correction and 309 extension tests pass locally | Install reviewed VSIX, then explicit real-service inventory retry; full installed-GUI migration/all 64 target ranges remain pending; Gremlin-shaped NoSQL is not Gremlin API qualification | blocked |
