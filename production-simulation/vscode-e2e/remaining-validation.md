...
| B05 | Cosmos explicit and Gremlin documents | Explicit-document base P1 passed; partition-preserving offline P1 passed; target verifier checks actual composite IDs/endpoints; tiny real-AGE and profile-bound controller tests pass; pinned candidate installed/reloaded; isolated source preparation r2 active after retained pre-write gateway refusal | Finish and verify isolated source preparation, remove temporary writer/stop preparation VM, then full installed-GUI migration and all 64 target ranges; new migration runner/verifier not deployed | not-run |
