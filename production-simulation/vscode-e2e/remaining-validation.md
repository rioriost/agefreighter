...
| B05 | Cosmos explicit and Gremlin documents | Explicit-document base P1 passed; partition-preserving offline P1 passed; target verifier now checks actual composite IDs/endpoints in a read-only snapshot; tiny real-AGE contract and profile-bound GUI controller tests pass | Pin/package corrected artifacts, refresh cloud gates, then full installed-GUI Gremlin migration and all 64 target ranges; local changes not installed/deployed | not-run |
