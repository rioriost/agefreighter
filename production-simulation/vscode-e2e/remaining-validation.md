...
B04 still needs live FK recommendation coverage and exact CSV choice bindings;
keep these distinct from the accepted catalog-to-mapped-inventory slice.
...
| B04 | Neo4j versions; PostgreSQL recommendations/review; CSV typed mapping | Base/manual routes have full P1 canonical PASS; September 20 installed GUI/Linux catalog discovered 18 tables, imported the sealed report and explicitly adopted nine vertex candidates; GUI edits/manual edges match all 18 frozen mappings and survive Reload Window/reconnect; complete inventory/import passes all 18 counts (1.6M vertices + 4M edges), with original seal independently matched | Fixture has no FKs; live FK recommendations remain unproven. Finish exact CSV choice bindings | partial |
