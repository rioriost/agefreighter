...
| B05 | Cosmos explicit and Gremlin documents | Explicit-document base P1 passed; partition-preserving offline P1 passed; target verifier checks actual composite IDs/endpoints; tiny real-AGE and profile-bound controller tests pass; pinned candidate installed/reloaded; isolated source r2 PASS with all 18 counts and 5.6M remote rows, writer removed and VM deallocated | Full installed-GUI migration and all 64 target ranges; new migration runner/verifier not deployed; Gremlin-shaped NoSQL fixture is not Gremlin API qualification | not-run |
