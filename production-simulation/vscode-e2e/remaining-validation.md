...
| B05 | Cosmos explicit and Gremlin documents | Explicit-document base P1 passed; partition-preserving offline P1 and isolated source preparation pass; target verifier checks composite IDs/endpoints; fresh installed-GUI runner/Reader pass, but inventory failed with opaque mapping-resolution error; separate discovery-cap defect now corrected locally with exclusion-based enumeration and safe error categories; Go/race tests and 300 extension tests pass | New pinned Linux artifact approval and explicit real-service inventory retry; full installed-GUI migration/all 64 target ranges remain pending; Gremlin-shaped NoSQL is not Gremlin API qualification | blocked |
