...
| B06 | Supported Cosmos authentication and RBAC propagation | Fixed managed-identity GUI scope documented; existing live account-scoped Reader/security rechecked; trust, approval binding and VM principal guards corrected with simulated denial/no-retry coverage | Install/retest corrected candidate; live data-plane denial/propagation evidence still missing; do not claim other CLI modes GUI-tested | partial |
