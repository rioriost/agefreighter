...
| B05 | Cosmos explicit and Gremlin documents | Explicit-document base P1 passed; partition-preserving offline P1 and isolated source preparation pass; target verifier checks composite IDs/endpoints; fresh installed-GUI runner/Reader pass, but inventory failed with opaque mapping-resolution error; bounded live reads additionally prove duplicate-heavy discovery exceeds its 10,000-document cap | Correct exact catalog discovery and safe error classification, then fresh reviewed artifact and explicit inventory retry; full installed-GUI migration/all 64 target ranges remain pending; Gremlin-shaped NoSQL is not Gremlin API qualification | blocked |
...
| B10 | Close/reload during assessment/load/verification; no replay | Persisted-state/process-exit tests pass; installed panels closed/reopened during inventory, load and full P1 verification, preserving operation IDs; actual terminal-state Reload Window/reconnect preserves records | Actual Extension Host crash/reload during active operations remains distinct and untested | partial |
