#!/usr/bin/env bash
# Fixture harness only. Never run on an original qualified source.
set -euo pipefail
umask 077
ip=10.246.3.5
root=/var/lib/af-op-n44
tls="$root/tls"
...
test -d /var/lib/neo4j44-data/databases/neo4j
test ! -e "$root"
test "$(docker inspect --format '{{.State.OOMKilled}}' af-n44-neo4j44)" = false
docker image inspect "$image" >/dev/null
install -d -m 700 "$root" "$root/evidence"
chmod 711 "$root"
docker inspect --format '{{.Image}} {{.State.Status}} {{.RestartCount}}' af-n44-neo4j44 > "$root/evidence/copied-container-before.txt"
docker update --restart=no af-n44-neo4j44 >/dev/null
docker stop --time 120 af-n44-neo4j44 >/dev/null
install -d -m 755 "$tls"
openssl req -x509 -newkey rsa:3072 -nodes -sha256 -days 7 \
  -subj '/CN=AGEFreighter OP-N44 P1 Test CA' \
  -keyout "$root/ca.key" -out "$root/evidence/ca.crt" 2>/dev/null
openssl req -new -newkey rsa:3072 -nodes -sha256 \
  -subj '/CN=10.246.3.5' -keyout "$tls/private.key" -out "$root/server.csr" 2>/dev/null
printf 'subjectAltName=IP:10.246.3.5\nextendedKeyUsage=serverAuth\nkeyUsage=digitalSignature,keyEncipherment\nbasicConstraints=CA:FALSE\n' > "$root/server.ext"
...
docker run -d --name af-op-n44-neo4j44 --restart unless-stopped --publish 7687:7687 \
  --ulimit nofile=65536:65536 --volume /var/lib/neo4j44-data:/data --volume "$tls:/ssl/bolt:ro" \
...
for attempt in $(seq 1 60); do
  if openssl s_client -connect "$ip:7687" -verify_ip "$ip" -verify_return_error \
    -CAfile "$root/evidence/ca.crt" </dev/null > "$root/evidence/tls-handshake.txt" 2>&1; then
    grep -F 'Verify return code: 0 (ok)' "$root/evidence/tls-handshake.txt" >/dev/null && break
  fi
  test "$attempt" -lt 60
  sleep 2
done
docker inspect --format '{{.Image}} {{.State.Status}} {{.State.OOMKilled}} {{.RestartCount}}' af-op-n44-neo4j44 > "$root/evidence/container-after.txt"
...
printf 'OP-N44 clone TLS readiness passed; authenticated GUI assessment still required.\n'
